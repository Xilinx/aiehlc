...
static void buildHostBlockByCloning(func::FuncOp mainFunc, ModuleOp moduleOp) {
    ...
    if (!allCoreTileVals.empty() && calleeAttrs) {
        ...
        // Core S2MM receivers must be armed BEFORE shim MM2S senders: the credit-based
        // stream switch otherwise has no credits and the shim channel deadlocks on
        // STALL_STREAM. Shim S2MM (output flow) is safe anywhere.
